...
- **Next.js 16.2.4** (App Router, React 19.2.4, Server Components + Server Actions). Uses View Transitions via `next-view-transitions`.
- **Supabase** (`@supabase/ssr` ^0.5.2 + `@supabase/supabase-js` ^2.45) for auth (magic link), Postgres, storage. Local dev on port 54321 (API), 54322 (DB, Postgres 17).
- **Tailwind v4** with CSS-native `@theme` tokens (no `tailwind.config`). PostCSS via `@tailwindcss/postcss`.
- **Tiptap 3.22** (StarterKit + Image, Link, Placeholder, Typography, CharacterCount) for the memory editor.
- **Radix UI** primitives + **shadcn-style** wrappers in `components/ui/` (button, dialog, drawer, popover, dropdown, alert-dialog, tooltip, avatar, carousel, tabs, label, input, textarea, skeleton). `cmdk` for the command palette. `vaul` for the mobile drawer. `sonner` for toasts. `motion` (Framer Motion successor) with `LazyMotion` + `domAnimation`. `embla-carousel-react` for carousels. `next-themes` for light/dark.
...
- `next.config.ts` also configures a permanent redirect `/alumni` → `/highlanders` and `images.remotePatterns` for `*.supabase.co` plus the local `127.0.0.1:54321` / `localhost:54321` storage origins.
...
`proxy.ts` (re-exports `updateSession` from `lib/supabase/middleware.ts`) enforces a `PUBLIC_PATHS` prefix list: `/`, `/login`, `/auth/callback`, `/not-on-roster`, `/request-access`, `/memories`, `/search`, `/gallery`. Public matching is prefix-based (`pathname === p || pathname.startsWith(p + "/")`), so `/memories/[id]` is implicitly public. **`/highlanders` is auth-gated** despite living in the public-feeling app shell — anon visitors get bounced to `/login`. `/memories/new` is the one `GATED_EXCEPTIONS` entry (auth-required even though `/memories` is public). After auth, missing profile → `/not-on-roster` (and signs out); missing `onboarded_at` → `/onboarding/class`; onboarded users hitting `/login` or `/onboarding/*` → `/profile/me`.
...
- `memory_reactions (id, memory_id→memories cascade, author_id→profiles cascade, emoji ∈ {❤️}, created_at, unique(memory_id, author_id, emoji))` — narrowed to heart-only in migration `20260504200000_heart_only_and_feed_counts.sql` (CHECK now `emoji = '❤️'`; non-heart rows were deleted). Anon + authenticated read on published memories; owner-only insert/delete (toggle = delete + insert, no update policy). The `REACTION_EMOJIS` const in `components/memory/reactions.ts` is `["❤️"]`.
...
- `feed_memories(p_cursor_at, p_cursor_id, p_sort, p_roles[], p_eras[], p_limit, p_exclude_ids[])` — **keyset-paginated** (cursor on `(published_at, id)`), not offset-paginated. Returns memory columns + inlined safe author projection (full_name, slug, avatar_path, class_year, role) + per-row `comment_count`, `like_count`, and `viewer_reacted` (LATERAL aggregates added in `20260504200000_heart_only_and_feed_counts.sql`). `p_exclude_ids` skips the pinned IDs already rendered above the fold.
- `pinned_memories(p_roles[], p_eras[], p_limit default 3)` — SECURITY DEFINER, returns pinned published memories (max 3) honoring same filters, ordered by `pinned_at desc`. Same projection as `feed_memories` including counts + `viewer_reacted`.
- `memory_era_counts()` — SECURITY DEFINER, returns `(era, count)` rows for published memories. Used by the timeline rail / era filter UI.
...
- `ensure_profile_for_current_user()` — SECURITY DEFINER self-heal RPC for the rare case where the `handle_new_user` trigger fired before a roster row landed; idempotently creates the missing profile if the user's email is now on roster.
...
| Sprint                            | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Notes |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| 0 — foundations                   | **Done** for tokens / breakpoints / fluid type / safe-area utilities / fonts / view transitions / dark mode / reduced motion. `lib/responsive/useMediaQuery.ts` exists and is consumed by mobile filter sheets. **Not done**: container queries beyond `MemoryCard`, Lighthouse CI, Playwright device matrix.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 3 — gallery, directory, search    | **Mostly done.** `/highlanders` directory (renamed from `/alumni`, `/alumni` 308-redirected via `next.config.ts`) with role/year/q filters and sort modes (`DirectoryFilters` + mobile `DirectoryFiltersSheet`); `/gallery` from cover/inline media (`GalleryGrid`); `/search` consuming `search_memories`. Loading skeletons + NavProgress bar shipped. A `TimelineRail` component + era filter (`FilterSheet`, `memory_era_counts` RPC) ship on the feed. **Not done**: full year scrubber with decade pins, container query treatment for `TimelineRail`.                                                                                                                                                                                                                                                                                                                                          |
| 4 — comments, reactions, realtime | **Done.** `memory_comments` + `memory_reactions` tables (RLS, hidden-column trigger guard), `get_memory_thread` RPC, server actions in `app/(app)/memories/[id]/actions.ts` (`addComment`, `deleteComment`, `hideComment`, `toggleReaction`), `<MemoryThread/>` + `<ThreadShell/>` + `<InlineComposer/>` client islands on `/memories/[id]` with optimistic updates, sr-only live region, owner/admin moderation menu. Realtime via the `lib/realtime/useMemoryChannel.ts` hook on a `memory:{id}` channel handling `postgres_changes` for both tables. Reactions narrowed to heart-only; per-memory `like_count` / `comment_count` / `viewer_reacted` are folded into the feed RPCs so the feed cards show real counters without N+1 fetches. Composer is keyboard-aware via `visualViewport`. **Still not done:** comment editing, threaded replies, mentions/notifications/email digest, presence. |
...
- `lib/auth/` — `slug.ts` (`buildSlug` + only Vitest test in the repo, `slug.test.ts`), `profile.ts` (`ensureSlugUnique`).
- `lib/email/send.ts` — Resend wrappers for access-request approve / decline emails.
- `lib/memories/` — `extract-images.ts` (walks Tiptap JSON for image nodes → drives the `/gallery` and feed cover fallbacks), `sanitize-body.ts` (allow-list pass over Tiptap JSONB before persisting), `storage-paths.ts` (path builders for the three buckets, all rooted at `{auth.uid()}/...`).
- `lib/realtime/useMemoryChannel.ts` — single hook that subscribes a client island to the `memory:{id}` channel and dispatches `postgres_changes` for `memory_comments` + `memory_reactions`. Reuse for any future per-memory live feature (presence, live pin/unpin) before reaching for a new channel name.
- `lib/responsive/useMediaQuery.ts` — SSR-safe `matchMedia` hook used by mobile filter sheets.
- `lib/storage/` — `publicUrl.ts` (`getPublicUrl(bucket, path)` for the three public-read buckets), `avatars.ts` (`signedAvatarUrls` batch helper, retained for admin/editor surfaces), `upload-image.ts` (client-side upload helper used by the editor + avatar picker).
- `lib/supabase/` — `client.ts` (browser SSR client), `server.ts` (RSC / server action client), `middleware.ts` (proxy entry point — public path table + auth/onboarding gating lives here).
...
- Server Components are the default. Client components are scoped to the surfaces that need interaction or realtime (`MemoryEditor`, `MemoryCard` / `FeedCard` / `FeedList` / `FeedFilters` / `FeedRightRail` / `FilterSheet`, `MemoryThread` / `ThreadShell` / `InlineComposer`, `PinToggle`, `TimelineRail`, `RosterRow`, `EditProfileForm`, `NewMemoryClient`, `EditMemoryClient`, `AvatarUploadField`, `DirectoryFilters` / `DirectoryFiltersSheet`, `HighlanderCard`, `CommandPalette`, `TopNav` / `MobileNav` / `MemoryFab` / `NavProgress` / `ThemeToggle`). Don't move data-fetching to client unless realtime requires it.
